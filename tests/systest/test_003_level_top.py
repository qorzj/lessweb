...
class TestLevelBottom(unittest.TestCase):
    ...
    def test(self):
        ...
        resp = requests.post(url, {'a': '1', 'b': '1'})
        self.assertDictEqual(expect_dict, resp.json())

        expect_dict = {'a': False, 'b': 2}
        resp = requests.post(url, {'a': '0', 'b': '2'})
        self.assertDictEqual(expect_dict, resp.json())

        resp = requests.post(url, {'a': '0', 'b': '-1'})
        self.assertEqual(400, resp.status_code)
        self.assertDictEqual({'message': "invalid range for uint(): '-1'", 'param': 'b'}, resp.json())
